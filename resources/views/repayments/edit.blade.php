            ...
            <div class="page-content">
                <div class="row">
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form class="row needs-validation"
                                    action="{{ route('repayments.update', $repayment->slug) }}" novalidate
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                            ...
                                            <select name="detail_id" class="form-select" required>
                                                ...
                                                @foreach ($detail as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ $repayment->detail_id == $item->id ? 'selected' : '' }}>
                                                        {{ $item->address }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="mb-3 position-relative">
                                            ...
                                            <div>
                                                <input name="payment_date" type="date" class="form-control"
                                                    value="{{ old('payment_date', \Carbon\Carbon::parse($repayment->payment_date)->format('Y-m-d')) }}"
                                                    required />
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="col-xl-6">
                        <div class="card">
                            <div class="card-body">
                                <div id="paymentPlanTableContainer">
                                    <!-- Table will be inserted here via JS -->
                                </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            loanSelect.addEventListener("change", function() {
                ...
                `;
                    ...
                    paymentPlanTableContainer.innerHTML =
                        "<p class='text-muted'>Please select a valid loan to generate a plan.</p>";
                }
            });
        });
    </script>
