...
<body data-sidebar="dark">
                                                        ...
                                                        <tr>
                                                            <td colspan="4" class="text-center">
                                                                <div class="text-center" style="margin-top: 15px;">
                                                                    <h5>No Invoices Assigned Uploaded</h5>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    @else
                                                        @foreach ($documents as $item)
                                                            @php
                                                                $newDocumentTag = 'new-document-' . $item->id;
                                                            @endphp
                                                            <tr data-id="{{ $item->id }}">
                                                                <td>
                                                                    <b>{{ $item->supplier_name }}</b>
                                                                    @if (!session()->has($newDocumentTag))
                                                                        <span class="badge bg-warning text-dark new-document">None Worked On</span>
                                                                    @endif
                                                                    <br>
                                                                    <span style="font-size: 11px; color:rgb(112, 112, 112);">TPIN: {{ $item->tpin }}</span>
                                                                </td>

                                                                <td>
                                                                    <b>{{ $item->invoice_amount ?? 'none' }}</b>
                                                                    <br>
                                                                    <span style="font-size: 11px; color:rgb(112, 112, 112);">
                                                                        Amount Net of VAT: {{ $item->amount_nv ?? 'none'}}
                                                                    </span>
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <a href="{{ route('documents.show', $item->slug) }}" class="btn btn-info waves-effect waves-light show-btn" data-id="{{ $item->id }}">
                                                                        Show
                                                                    </a>
                                                                    <a href="{{ route('documents.edit', $item->slug) }}" class="btn btn-info waves-effect waves-light edit-btn" data-id="{{ $item->id }}">
                                                                        <i class="dripicons-document-edit"></i>
                                                                    </a>
                                                                    <button type="button" class="btn btn-danger waves-effect waves-light delete-btn" data-bs-toggle="modal"
                                                                        data-bs-target="#deleteMinistryModal" data-route="{{ route('documents.destroy', $item->slug) }}">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            let viewedDocuments = JSON.parse(localStorage.getItem("viewedDocuments")) || [];

            // Hide "New Document" tag for viewed documents
            document.querySelectorAll("tr[data-id]").forEach(row => {
                let documentId = row.getAttribute("data-id");
                if (viewedDocuments.includes(documentId)) {
                    let newTag = row.querySelector(".new-document");
                    if (newTag) newTag.style.display = "none";
                }
            });

            // Mark document as viewed when clicking Show/Edit
            document.querySelectorAll(".show-btn, .edit-btn").forEach(button => {
                button.addEventListener("click", function () {
                    let documentId = this.getAttribute("data-id");
                    if (!viewedDocuments.includes(documentId)) {
                        viewedDocuments.push(documentId);
