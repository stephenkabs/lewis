                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <h4 class="card-title"><a class="btn btn-info btn-sm waves-effect waves-light"
                                            href="/role"><i class="dripicons-arrow-thin-left"></i></a> <a
                                            class="btn btn-info btn-sm waves-effect waves-light" href="/permission/create">Add
                                            New</a></h4><br>

                                    <div class="table-rep-plugin">
                                        <div data-pattern="priority-columns">
                                            <div>
                                                <input type="text" id="permission-search" class="form-control mb-3"
                                            ...
                                            <table id="tech-companies-1" class="table table-striped">
                                                ...
                                                <tbody id="permission-table-body">
                                                    @foreach ($permissions as $item)
                                                        <tr>
                                                            <td><a
                                                                    class="btn btn-info btn-sm waves-effect">{{ $item->name }}</a>
                                                            </td>
                                                            <td>
                                            ...
                                            <div id="permission-pagination-container" class="mt-3"></div>

                                            <script>
                                                // JavaScript for Pagination and Real-Time Search
                                                document.addEventListener("DOMContentLoaded", function() {
                                                    const permissionTableBody = document.getElementById("permission-table-body");
                                                    const permissionRows = Array.from(permissionTableBody.querySelectorAll("tr"));
                                                    const permissionRowsPerPage = 5;
                                                    const permissionPaginationContainer = document.getElementById("permission-pagination-container");
                                                    const permissionSearchInput = document.getElementById("permission-search");

                                                    let currentPermissionPage = 1;

                                                    function displayPermissionPage(page) {
                                                        const start = (page - 1) * permissionRowsPerPage;
                                                        const end = start + permissionRowsPerPage;
                                                    ...
                                                    function setupPermissionPagination() {
                                                        const totalPages = Math.ceil(permissionRows.length / permissionRowsPerPage);
                                                        permissionPaginationContainer.innerHTML = "";

                                                        for (let i = 1; i <= totalPages; i++) {
                                                            const button = document.createElement("button");
                                                            button.textContent = i;
                                                            button.className =
                                                            `btn btn-sm ${i === currentPermissionPage ? "btn-primary" : "btn-secondary"}`;
                                                            button.addEventListener("click", () => {
                                                                currentPermissionPage = i;
                                                                displayPermissionPage(currentPermissionPage);
                                                                setupPermissionPagination();
                                                            });
                                                            permissionPaginationContainer.appendChild(button);
                                                        }
                                                    }

                                                    function filterPermissionRows() {
                                                        const query = permissionSearchInput.value.toLowerCase();

                                                        permissionRows.forEach(row => {
                                                            const permission = row.cells[0].textContent.toLowerCase();

                                                            if (permission.includes(query)) {
                                                                row.style.display = "";
                                                            } else {
                                                    ...
                                                    permissionSearchInput.addEventListener("input", filterPermissionRows);
                                            ...
                                            <style>
                                                #permission-pagination-container button {
                                                    margin-right: 8px;
