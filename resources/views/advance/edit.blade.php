                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <h4 class="card-title"><a class="btn btn-info waves-effect waves-light"
                                            ...
                                            action="{{ isset($advance) ? route('advance.update', $advance->slug) : route('advances.store') }}"
                                            method="POST"
                                            novalidate>
                                          @csrf
                                          @isset($advance)
                                              ...
                                              <select name="worker_id" class="form-control" required>
                                                  ...
                                                  @foreach ($workers as $worker)
                                                      <option value="{{ $worker->id }}"
                                                              {{ (isset($advance) && $advance->worker_id == $worker->id) ? 'selected' : '' }}>
                                                          {{ $worker->name }}
                                                      </option>
                                                  @endforeach
                                              </select>
                                          ...
                                          <div class="mb-3 position-relative">
                                              ...
                                              <textarea name="comment"
                                                        class="form-control"
                                                        placeholder="Enter Comment (Optional)">{{ old('comment', $advance->comment ?? '') }}</textarea>
                                          </div>

                                          <!-- Submit & Reset Buttons -->
                                          <div class="mb-0">
                                              <button type="submit" class="btn btn-info waves-effect waves-light">
                                                  {{ isset($advance) ? 'Update' : 'Save' }}
                                              </button>
